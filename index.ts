...
(function () {
  ...
  const taskTodo = document.getElementById(
    "tasks-todo",
  ) as HTMLDivElement | null;
  const taskProgress = document.getElementById(
    "tasks-in-progress",
  ) as HTMLDivElement | null;
  const taskComplete = document.getElementById(
    "tasks-completed",
  ) as HTMLDivElement | null;
  const taskAppear = document.getElementById(
    "columns-container",
  ) as HTMLDivElement | null;
  const modal = document.getElementById("modal-show") as HTMLDivElement | null;
  const taskModel = document.getElementById(
    "task-modal",
  ) as HTMLDivElement | null;
  const modelTitle = document.getElementById(
    "modal-title",
  ) as HTMLHeadingElement | null;
  const modelIcon = document.querySelector("#modal-icon") as HTMLElement | null;
  const modelClose = document.querySelector(
    "#close-modal-btn",
  ) as HTMLButtonElement | null;
  const modelForm = document.getElementById(
    "task-form",
  ) as HTMLFormElement | null;
  const modelFormTitle = document.getElementById(
    "task-title",
  ) as HTMLInputElement | null;
  const modelFormError = document.getElementById(
    "title-error",
  ) as HTMLElement | null;
  const modelFormProf = document.getElementById(
    "task-priority",
  ) as HTMLSelectElement | null;
  const modelFormDate = document.getElementById(
    "task-due-date",
  ) as HTMLInputElement | null;
  const modelFormDateError = document.getElementById(
    "date-error",
  ) as HTMLElement | null;
  const modelFormDesc = document.getElementById(
    "task-description",
  ) as HTMLTextAreaElement | null;
  const modelFormDescError = document.getElementById(
    "description-error",
  ) as HTMLElement | null;
  const modelFormDescCount = document.getElementById(
    "char-count",
  ) as HTMLElement | null;
  const cancelBtn = document.getElementById(
    "cancel-btn",
  ) as HTMLButtonElement | null;
  const sumbitBtn = document.querySelector(
    "#submit-btn",
  ) as HTMLButtonElement | null; // Note: kept original typo "sumbitBtn"
  ...
  function dueTime(d: string): number {
  ...
  function isDueSoon(d: string): boolean {
  ...
  function emptyHTML(): string {
    return `<div class="flex flex-col items-center justify-center py-12 text-slate-400"> <i class="fa-regular fa-folder-open text-4xl mb-3 opacity-50"></i> <p class="text-sm">No tasks yet</p> <p class="text-xs mt-1">Click + to add one</p> </div>`;
  }
  ...
  /* ---------- errors ---------- */
  function showTitleError(msg: string): void {
    if (modelFormError) {
  ...
  /* ---------- task card ---------- */
  function createTaskCardHTML(t: Task): string {
    ...
    return `<div class="group bg-white rounded-xl p-4 shadow-sm border border-slate-100 hover:shadow-md hover:border-slate-200 transition-all duration-200 ${t.status === "completed" ? "opacity-75" : ""} ${over ? "ring-2 ring-red-100 border-red-200" : ""}" data-task-id="${t.id}">
      ...
      <h3 class="font-semibold mb-2 ${t.status === "completed" ? "line-through text-slate-500" : "text-slate-800"}">${escapeHtml(t.title)}</h3>
      ...
      <div class="flex flex-wrap items-center gap-2 mb-4">${badges}
        ${t.status === "completed" ? `<span class="bg-emerald-100 text-emerald-600 text-[10px] font-semibold px-2 py-1 rounded-full uppercase tracking-wide flex items-center gap-1"><i class="fa-solid fa-check"></i> Done</span>` : ""}
      </div>
      ${dateRow}
  ...
  function renderColumns(): void {
    const columnElements: Record<string, HTMLDivElement | null> = {
      todo: taskTodo,
      "in-progress": taskProgress,
      completed: taskComplete,
    };

    const allStatuses: Array<Task["status"]> = [
      "todo",
      "in-progress",
      "completed",
    ];
  ...
  /* ---------- modal ---------- */
  function setSubmitMode(edit: boolean): void {
    if (modelTitle)
  ...
  function openModal(taskId: number | null = null): void {
    if (
      !modal ||
      !modelForm ||
      !modelFormTitle ||
  ...
  function handleSubmit(e: Event): void {
    ...
    const priorityValue = modelFormProf.value as Task["priority"];
  ...
  if (sumbitBtn) {
    sumbitBtn.addEventListener("click", function (e: MouseEvent) {
      e.preventDefault();
      handleSubmit(e);
    });
  }
  ...
  if (taskAppear) {
    taskAppear.addEventListener("click", function (e: MouseEvent) {
      const target = e.target as HTMLElement;

      const del = target.closest(".delete-btn") as HTMLButtonElement | null;
      if (del) {
      ...
      const edit = target.closest(".edit-btn") as HTMLButtonElement | null;
      if (edit) {
        openModal(Number(edit.getAttribute("data-task-id")));
        return;
      }

      const move = target.closest(".move-btn") as HTMLButtonElement | null;
      if (move) {
        const id = Number(move.getAttribute("data-task-id"));
        const go = move.getAttribute("data-target") as Task["status"] | null;
        if (!go) return;
        for (const t of tasks) {
          if (t.id === id) {
            t.status = go;
